...
const workItemFormSchema = z.object({
  ...
  // Actual hours is required when status is DONE, but only for leaf items (TASK, BUG)
  ...
  return true; // EPIC, FEATURE, STORY are auto-calculated when status is DONE
}, {
  message: "Actual hours is required when status is DONE",
  path: ["actualHours"],
}).refine((data) => {
  // Estimate is required only for TASK and BUG types (not for auto-calculated parent items)
  if (['TASK', 'BUG'].includes(data.type)) {
    return data.estimate && data.estimate.trim().length > 0;
  }
  return true; // EPIC, FEATURE, STORY are auto-calculated
}, {
...
export function EditItemModal({
  ...
  const watchedActualHours = form.watch("actualHours");
  ...
  useEffect(() => {
    if (displayWorkItem && isOpen) {
      ...
      // Handle actual hours - ensure proper conversion from number to string
      const actualHoursValue = displayWorkItem.actualHours !== undefined && displayWorkItem.actualHours !== null
        ? displayWorkItem.actualHours.toString()
        : "";

      // Handle bug fields - ensure proper fallback values
      const currentBehaviorValue = displayWorkItem.currentBehavior ?? "";
      const expectedBehaviorValue = displayWorkItem.expectedBehavior ?? "";
      const bugTypeValue = displayWorkItem.bugType ?? "BUG";
      const severityValue = displayWorkItem.severity ?? "LOW";
  ...
  const onSubmit = async (data: WorkItemFormValues) => {
    ...
    try {
      ...
      console.log("Raw tag';;;;;;;;;;;;;;;;;;;;;;;;;s value:", data.tags);
      ...
      // Invalidate the specific work item query to refresh screenshot data
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
            ...
            {workItem?.type === "BUG" && (
              <div className="space-y-4 bg-blue-50 p-4 rounded border border-blue-200" key={`bug-fields-${workItem?.id}`}>
                ...
                {/* Current & Expected Behavior - only show for DEFECT and PROD_INCIDENT */}
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                                <Textarea 
                                  ...
                                  rows={2} 
                                  className={`text-sm ${!field.value ? "border-orange-500 ring-2 ring-orange-300 bg-orange-50" : ""}`}
                                  value={field.value ?? ""} 
                                  onChange={(e) => {
                                    field.onChange(e);
                            ...
                            {selectedFile ? (
                              ...
                              <div className="relative group">
                                <div className="flex flex-col items-start">
                                  <img 
                                    src={getScreenshotUrl((displayWorkItem as any).screenshotBlob || (displayWorkItem as any).screenshot || (displayWorkItem as any).screenshot_blob)} 
                                    alt="Screenshot" 
                                    className="max-h-48 max-w-full border rounded cursor-zoom-in object-contain" 
                                    onClick={(e) => {
                                      e.preventDefault();
                                      e.stopPropagation();
                                      const screenshotData = (displayWorkItem as any).screenshotBlob || (displayWorkItem as any).screenshot || (displayWorkItem as any).screenshot_blob;
                                      console.log('Screenshot data:', screenshotData);
                                      
                                      if (screenshotData) {
                                        if (screenshotData.startsWith('data:')) {
                                          // For data URLs, create a blob and open that instead
                                          try {
                                            const response = fetch(screenshotData);
                                            ...
                                            // Fallback: create a modal or show in current window
                                            const newWindow = window.open('', '_blank');
                                            if (newWindow) {
                                              newWindow.document.write(`<html><body style="margin:0;background:#000;display:flex;justify-content:center;align-items:center;min-height:100vh;"><img src="${screenshotData}" style="max-width:100%;max-height:100vh;object-fit:contain;" /></body></html>`);
                                            }
                                          }
                                        } else {
                                          // For regular URLs, use the screenshot utility
                                          const screenshotUrl = getScreenshotUrl(screenshotData);
                                        ...
                                        console.error('No valid screenshot data to open');
                                      }
                                    }}
                                    onError={(e) => {
                                      console.error('Failed to load screenshot:', (displayWorkItem as any).screenshotBlob || (displayWorkItem as any).screenshot || (displayWorkItem as any).screenshot_blob);
                                  ...
                                  />
                                  <p className="text-sm mt-1 text-gray-600">Drag and drop or click to change</p>
                                </div>
                                <Button
                                  type="button"
                                  variant="destructive"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="grid grid-cols-2 gap-4">
                <FormField
                  ...
                  name="estimate"
                  render={({ field }) => {
                    const isParentItem = ['STORY', 'FEATURE', 'EPIC'].includes(workItem.type);
                    const calculatedEstimate = isParentItem && workItem?.id 
                      ? calculateTotalEstimatedHours(workItem.id, workItem.type)
                      : 0;
                    
                    return (
                      <FormItem>
                        <FormLabel>
                          {getEstimateLabel()} {isParentItem ? "(Auto-calculated)" : <span className="text-red-500">*</span>}
                        </FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                            placeholder={workItem.type === "STORY" ? "Story points" : "Hours"}
                            value={isParentItem && calculatedEstimate > 0 
                              ? calculatedEstimate.toFixed(1) 
                              : field.value || ""
                            }
                            disabled={isParentItem}
                            className={isParentItem ? "font-bold text-blue-600 bg-blue-50 border-blue-200" : ""}
                          />
                        </FormControl>
                        {isParentItem && (
                          <FormDescription className="text-[10px]">
                            {calculatedEstimate > 0 
                              ? `Sum of all child items: ${calculatedEstimate.toFixed(1)} hours`
                              : "Sum of all child items (no child items with estimates found)."
                            }
                          </FormDescription>
                        )}
                        <FormMessage />
                ...
                <FormField
                  ...
                  name="actualHours"
                  render={({ field }) => {
                    const isDone = watchedStatus === "DONE";
                    const hasValue = field.value !== undefined && field.value !== null && field.value !== "";
                    const isParentItem = ['STORY', 'FEATURE', 'EPIC'].includes(workItem.type);
                    const calculatedActualHours = isParentItem && workItem?.id 
                      ? calculateTotalActualHours(workItem.id, workItem.type)
                      : 0;
                    ...
                    return (
                      <FormItem>
                        <FormLabel>
                          Actual Hours
                          {isParentItem ? " (Auto-calculated)" : isDone && <span className="text-red-500">*</span>}
                        </FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            step="0.1"
                            placeholder="Hours"
                            value={isParentItem && calculatedActualHours > 0 
                              ? calculatedActualHours.toFixed(1) 
                              : field.value ?? ''
                            }
                            onChange={(e) => field.onChange(e.target.value)}
                            disabled={isParentItem}
                            className={isParentItem 
                              ? "font-bold text-orange-600 bg-orange-50 border-orange-200" 
                              : isDone && !hasValue 
                        ...
                        {isParentItem ? (
                          <FormDescription className="text-[10px]">
                            {calculatedActualHours > 0 
                              ? `Sum of all child items: ${calculatedActualHours.toFixed(1)} hours`
                              : "Sum of all child items (no child items with actual hours found)."
                            }
                          </FormDescription>
                        ) : (isDone && !hasValue) ? (
                          <FormDescription className="text-orange-600 font-medium text-[11px]">
