...
function canUserCreateType(type: string, currentUser: any): boolean {
  ...
  if (currentUser.role === 'ADMIN' || currentUser.role === 'SCRUM_MASTER' || currentUser.role === 'PROJECT_MANAGER') {
    return true;
  }
...
export default function ProjectDetails() {
  ...
  const { data: workItems = [], refetch: refetchWorkItems } = useQuery<WorkItem[]>({
    ...
    queryFn: async () => {
      const response = await apiGet(`/projects/${projectId}/work-items`);
      
      // Debug log the work items to see actual hours data
      console.log('=== WORK ITEMS API RESPONSE ===');
      console.log('Total items:', response?.length || 0);
      if (response?.length > 0) {
        const sampleItems = response.slice(0, 3).map(item => ({
          id: item.id,
          title: item.title?.substring(0, 30) + '...',
          actualHours: item.actualHours,
          actualHoursType: typeof item.actualHours,
          status: item.status
        }));
        console.log('Sample items with actualHours:', sampleItems);
      }
                  ...
                  <table className="w-full border-collapse text-xs min-w-[1000px]">
                    <thead>
                      <tr className="text-left bg-neutral-100 text-xs text-neutral-700 border-b border-neutral-200">
                        <th className="font-medium px-2 py-1 border-r border-neutral-200">ID</th>
                        <th className="font-medium px-2 py-1 border-r border-neutral-200">Title</th>
                        <th className="font-medium px-2 py-1 border-r border-neutral-200">Type</th>
                        <th className="font-medium px-2 py-1 border-r border-neutral-200">Status</th>
                        <th className="font-medium px-2 py-1 border-r border-neutral-200">Priority</th>
                        ...
                        return Array.isArray(workItems) && workItems
                          ...
                          .map(item => (
                            <tr key={item.id} className="border-b border-neutral-200 hover:bg-neutral-50 text-xs">
                              <td className="px-2 py-0.5 border-r border-neutral-200">{item.externalId}</td>
                              ...
                              <td className={`px-2 py-0.5 border-r border-neutral-200 ${blinkingItemId === item.id ? 'animate-blink-cell' : ''}`}>
                                <span className={`text-xs text-center ${(item.type === 'EPIC' || item.type === 'FEATURE' || item.type === 'STORY')
                                ...
                                {(item.updatedBy || item.updatedByName) && item.updatedAt ? (
                                  <div className="flex items-center space-x-1">
                                    ...
                                    <div className="flex items-center space-x-1 min-w-0">
                                      <span className="text-[10px] truncate max-w-[80px] flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <div>
                        <h3 className="text-sm font-semibold text-gray-900">
                          {currentUser?.role === 'ADMIN' || currentUser?.role === 'SCRUM_MASTER' || currentUser?.role === 'PROJECT_MANAGER'
