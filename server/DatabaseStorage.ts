...
export class DatabaseStorage {
  ...
  async cleanupExpiredProjectMembers(): Promise<number> {
    if (!db) return 0;
    const now = new Date();
    const result = await db
      .delete(projectMembers)
  ...
  async getProjectsForUser(userId: number, userRole: string): Promise<Project[]> {
    if (!db) return [];
    ...
    // Get all project memberships for this user (including expired ones for now)
    ...
    const now = new Date();
    const validProjectMemberIds = userProjectMemberships
      .filter(pm => !pm.expiresAt || new Date(pm.expiresAt) > now)
      .map(pm => pm.projectId);
    ...
    // Get projects where:
    // 1. User is a team member (teamId in user's teams)
    // 2. User is a project member with valid (non-expired) access
    const allProjects = await db.select().from(projects);
    
    const accessibleProjects = allProjects.filter(project => {
      // Include if user is in the project's team
      if (project.teamId && teamIds.includes(project.teamId)) {
        return true;
      }
      // Include if user is a direct project member with valid access
  ...
  async updateProject(id: number, updates: Partial<Project>): Promise<Project | undefined> {
    if (!db) return undefined;
    try {
      // Build the update object, preserving explicit null values
      const updateData: any = {};
      for (const [key, value] of Object.entries(updates)) {
        // Explicitly include null values (don't filter them out)
  ...
  async getWorkItemsByProjectWithTeamFilter(
    projectId: number,
    userId: number,
    userRole: string
  ): Promise<WorkItem[]> {
    ...
    if (userTeams.length === 0) {
      return []; // User is not a member of any team
    }
    ...
    if (!project || !project.teamId) {
      return []; // Project has no team assigned
    }
    
    // Check if user is a member of the project's team
    if (!teamIds.includes(project.teamId)) {
      return []; // User is not a member of the project's team
    }
    
    // User has access to this project's team, return all work items
    return this.getWorkItemsByProject(projectId);
  }
